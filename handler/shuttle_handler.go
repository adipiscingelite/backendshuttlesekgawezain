...
func (h *ShuttleHandler) GetRecapShuttle(c *fiber.Ctx) error {
	log.Println("Handler accessed") // Pindahkan ke sini
	userUUID, ok := c.Locals("userUUID").(string)
	if !ok || userUUID == "" {
			return utils.BadRequestResponse(c, "Invalid or missing userUUID", nil)
	}
	log.Printf("Extracted userUUID: %s", userUUID)
	...
	shuttles, err := h.ShuttleService.GetRecapShuttle(parentUUID)
	if err != nil {
		return utils.NotFoundResponse(c, "Shuttle data not found", nil)
	}
